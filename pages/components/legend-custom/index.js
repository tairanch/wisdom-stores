function initChart (canvas, width, height, F2) {
  ...
  chart.interval().position('name*score').color('name', ['#BB8BFF', '#A26FFF']);
  chart.line().position('name*avgScore').shape('smooth').color('#000');
  chart.point().position('name*avgScore').shape('smooth').size(3).style({
    fill: '#000',
    stroke: '#fff',
    lineWidth: 1
  });
